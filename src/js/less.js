...
(function (factory, undefined) {
  ...
  function less(term, text, options) {
    ...
    function print() {
      ...
      var should_substring = options.wrap
        ? false
        : to_print.filter(function (line) {
        ...
        for (var j = 0, jlen = line.length; j < jlen; ++j) {
          if (line[j] === "[" && line[j + 1] === "[") {
            ...
            if (line.substring(j).match(start_re)) {
              ...
              line =
                line.substring(0, j) + line.substring(j).replace(start_re, rep);
              j += rep.length - 2;
              if (i >= pos && index === -1) {
                index = pos = i;
              }
    ...
    term.push($.noop, {
      ...
      keydown: function (e) {
        ...
        if (term.get_prompt() !== "/") {
          if (key === "/") {
            term.set_prompt("/");
          } else if (in_search && $.inArray(e.which, [78, 80]) !== -1) {
            if (key === "N") {
              // search_string
              if (last_found !== -1) {
