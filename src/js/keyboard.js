(function (global) {
  ...
  [
    ...
    if (!(p[0] in global.KeyboardEvent)) global.KeyboardEvent[p[0]] = p[1];
  });
  ...
  try {
    var nativeLocation =
      nativeKeyboardEvent && "location" in new KeyboardEvent("");
  } catch (_) {}

  function keyInfoForEvent(event) {
    var keyCode =
      "keyCode" in event ? event.keyCode : "which" in event ? event.which : 0;
    var keyInfo = (function () {
      ...
      if (
        "keyIdentifier" in event &&
        event.keyIdentifier in keyIdentifierTable
      ) {
